...
const sendAssignedEmailClient = async (
  email,
  name,
  lastname,
  pname,
...
module.exports.accept_order = async (req, res) => {
  ...
  try {
    ...
    if (!order.client_confirmed) {
      console.log("huna");
      res.status(400).json({
        message: "Please wait until the client has confirmed their order",
        token,
      });
      return;
    }
  ...
  try {
    for (let i = 0; i < traveler.new_orders.length; i++) {
      ...
      let mailOptions = {
        ...
        text:
          "Dear " +
          (await User.findById(order.client).name) +
          " " +
          (await User.findById(order.client).lastname) +
          ",\n\n" +
    ...
    traveler.canceled_orders.concat(
...
module.exports.markshipped = async (req, res) => {
  ...
  if (order) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Order marked as shipped", token });
    } catch (err) {
      console.log(err);
...
module.exports.markarrived = async (req, res) => {
  ...
  if (order) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Order marked as arrived", token });
    } catch (err) {
      console.log(err);
      res.status(500).json({ message: "Server Error Occured", token });
    }
...
module.exports.editProfile = async (req, res) => {
  ...
  if (trav) {
    try {
      const trav = await Traveler.findByIdAndUpdate(travId, req.body, {
        new: true,
      });
      res.send({ trav, token });
    } catch (error) {
