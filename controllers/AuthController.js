...
const handleErrors = (err) => {
  ...
  if (err.message === "user blocked") {
    errors.email =
      "This account has been freezed due to excess of failed login attempts. Try again later.";
    return errors;
  }
  ...
  if (err.message.includes("user validation failed")) {
    ...
    Object.values(err.errors).forEach(({ properties }) => {
      //similar to forEach(err) then err.properties
...
const createEmailLink = (id) => {
  const emailToken = jwt.sign({ id }, process.env.SECRET_EMAIL, {
    expiresIn: "2d",
  });
...
module.exports.login_post = async (req, res) => {
  ...
  try {
    ...
    if (user && !user.valid_e) {
      return res
        .status(406)
        .json({ message: "The email is not yet confirmed" });
    }
    if (user) {
      console.log(user);
      ...
      res
        .status(200)
        .json({ user: user._id, type: user.type, token: accessToken });
    } else {
      console.log("hi");
      ...
      if (traveler) {
        if (traveler.approved == false) {
          res.status(400).json({
            message: "This account has still to be approved by the admin",
          });
          return;
        }
        ...
        if (trav.revoked) {
          return res
            .status(400)
            .json({ message: "This account has been revoked by the admin" });
        }
        if (trav) {
          console.log(trav);
          ...
          res
            .status(200)
            .json({ user: trav._id, type: trav.type, token: accessToken });
        }
      } else {
        if (email !== "admin") {
