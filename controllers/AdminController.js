...
const sendOrderAcceptEmail = async (
  email,
  name,
  lastname,
  pname,
...
module.exports.getActiveTravelers_get = async (req, res) => {
  try {
    const activeTravelers = await Traveler.find({
      active: true,
      revoked: false,
      ticket: { $ne: null, $ne: undefined },
    });
  ...
  if (order && traveler && traveler.active == true && !traveler.revoked) {
    try {
      ...
      return res
        .status(200)
        .json(
          "Successfully Assigned Order & sent email to client and traveler"
        );
    } catch (err) {
      console.log(err);
    }
  } else {
    return res.status(404).json({ message: "Order or Traveler not found." });
  }
};
  ...
  if (order) {
    try {
      ...
      if (
        ...
        res
          .status(200)
          .json({ message: "Order has been successfully rejecetd" });
      } else {
        res.status(418).json({
          message: "Cannot reject order as it has already been processed.",
        });
      }
    } catch (err) {
      console.log(err);
  ...
  if (order) {
    try {
      ...
      if (traveler.new_orders.includes(orderId) && order.waiting_resp == true) {
        order.traveler = null;
        (order.waiting_resp = false), order.save();
        ...
        res
          .status(200)
          .json({ message: "Order has been successfully revoked" });
      }
    } catch (err) {
      console.log(err);
    }
...
module.exports.setcost_post = async (req, res) => {
  ...
  if (order) {
    try {
      ...
      res
        .status(200)
        .json({ message: "Order cost has been successfully updated." });
    } catch (err) {}
  } else {
    res.status(400).json({ message: "Order not found" });
  }
...
module.exports.deleteTraveler = async (req, res) => {
  ...
  try {
    ...
    if (traveler) {
      ...
      res
        .status(200)
        .json({ message: "Traveler has been successfully deleted." });
    } else {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while deleting the traveler." });
  }
};
...
module.exports.unrevoke_post = async (req, res) => {
  ...
  try {
    if (traveler) {
      ...
      res
        .status(200)
        .json({ traveler, message: "Successfully Unrevoked Access" });
    } else {
      res.status(404).json({ message: "Traveler not found." });
    }
  } catch (err) {
...
module.exports.setTestimonial_post = async (req, res) => {
  try {
    ...
    if (feedback) {
      ...
      res
        .status(200)
        .json({ message: "Testimonial has been successfully set." });
    } else {
  ...
  try {
    ...
    if (order) {
      if (order.status == 5) {
        ...
        return res
          .status(400)
          .json({ message: "Order is not at arrived stage " });
      }
      return res
        .status(200)
        .json({ message: "Order has been successfully marked as sent." });
    } else {
      res.status(404).json({ message: "Order not found." });
    }
  } catch (err) {
...
module.exports.acceptTraveler_post = async (req, res) => {
  ...
  if (!trav) res.send("not registered");
    ...
    res
      .status(200)
      .json({ message: "Traveler has been successfully accepted." });
  }
};
