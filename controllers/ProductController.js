...
function extractProductInfo(str) {
  ...
  if (weightMatch) {
    ...
    if (
      weightMatch[3].toLowerCase() === "ounces" ||
      weightMatch[3].toLowerCase() === "oz"
    ) {
      weight /= 16;
    }
  }
...
async function getData(youRL) {
  ...
  if (price == undefined) {
    price = $("span[id='price_inside_buybox']").text();
  }
  ...
  let dimensions;
  $("div[id^='detailBullets_feature'] span[class='a-list-item']").each(
    (i, elm) => {
      let dd = $(elm).text();
      if (
        dd.includes("Package Dimensions") ||
        dd.includes("Product Dimensions")
      ) {
        dimensions = dd.trim();
        return false;
      }
  ...
  if (dimensions == undefined) {
    $(
      ...
      let ddd = $(elm).text();
      if (
        ddd.includes("Package Dimensions") ||
        ddd.includes("Product Dimensions") ||
        ddd.includes(" x ")
      ) {
        dimensions = ddd.trim();
        return false;
  ...
  if (dimensions == undefined) {
    $(
      ...
      let dddd = $(elm).text();
      if (
        dddd.includes("Package Dimensions") ||
        dddd.includes("Product Dimensions") ||
        dddd.includes(" x ")
      ) {
  ...
  if (dimensions) {
    ...
    const weight = dim.substring(
      dim.indexOf("Weight: ") + 8,
      dim.indexOf(" Length")
    );
    const length = dim.substring(
      dim.indexOf("Length: ") + 8,
      dim.indexOf(" Width")
    );
    const width = dim.substring(
      dim.indexOf("Width: ") + 7,
      dim.indexOf(" Height")
    );
  ...
  getData(productLink).then((data) => {
    if (data.length == -1 || data.height == -1 || data.width == -1) {
      ...
      const details = {
        ...
        imageSource: data.imageSource,
        dimensions:
          data.length + " x " + data.width + " x " + data.height + " in",
        weight: data.weight,
        length: data.length,
        width: data.width,
        height: data.height,
    ...
    if (product) {
      ...
      if (copyorder.cost) {
        ...
        const order = new Order({
          ...
          a_commission:
            (copyorder.a_commission / copyorder.quantity) * parseInt(quantity),
          t_commission:
            (copyorder.t_commission / copyorder.quantity) * parseInt(quantity),
        ...
        product.quantity_ordered =
          product.quantity_ordered + parseInt(quantity);
        await product.save();
        return res.status(200).json({
          message:
            "Successfully Created Order with already existing product and its cost.",
          accessToken: newNAT,
        });
      } else {
        ...
        await product.save();
        return res.status(200).json({
          message: "Successfully Created Order with already existing product.",
          accessToken: newNAT,
        });
      }
    } else {
      ...
      if (
        ...
        const t_commission =
          (newProduct.length + newProduct.height + newProduct.width) * 2 +
          newProduct.weight * 2;
        ...
        await client.save();
        return res.status(200).json({
          message: "Successfully Created Order with new Product WITH cost.",
          accessToken: newNAT,
        });
      } else {
