  ...
  if (payload) {
    try {
      ...
      if (order.status == 1 && order.traveler != null) {
        ...
        res
          .status(400)
          .json({ message: "Order has not yet been approved by an admin" });
      }
    } catch (err) {
      console.log(err);
    }
...
module.exports.complete_order_post = async (req, res) => {
  ...
  if (
    client &&
    order &&
    client.active_orders.includes(orderId) &&
    order.status == 6
  ) {
    try {
      order.status = 7;
      ...
      await client.save();
      if (
        traveler.new_orders.length == 0 &&
        traveler.assigned_orders.length == 0
      ) {
        traveler.active = false;
        traveler.provided_pickup = "";
      }
      await traveler.save();
      sendCompletiontoTraveler(
        traveler.email,
        traveler.name,
        traveler.lastname,
        prod.title
...
module.exports.editProfile = async (req, res) => {
  ...
  if (client) {
    try {
      const user = await User.findByIdAndUpdate(clientId, req.body, {
        new: true,
      });
      res.send({ user, token });
    } catch (error) {
      ...
      if (auth) {
        ...
        bcrypt.genSalt(10, function (err, salt) {
          ...
          bcrypt.hash(newPassword, salt, async function (err, hash) {
            ...
            return res
              .status(200)
              .json({ message: "Successfully changed password", token });
          });
        });
      } else {
...
module.exports.giveFeedback_post = async (req, res) => {
  ...
  const orderId = req.params.orderid;
  const order = await Order.findById(orderId);
  const { rating, arrived_on_time, as_described, good_service, message } =
    req.body;
  if (order) {
    try {
